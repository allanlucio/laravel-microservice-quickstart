import { Actions, AddUploadAction, Upload, RemoveUploadAction, UpdateProgressAction, SetUploadErrorAction } from "./types";
...
function removeUpload(state: State = INITIAL_STATE, action: RemoveUploadAction): State{
...
function setUploadError(state: State = INITIAL_STATE, action: SetUploadErrorAction): State{
    const videoId = action.payload.video.id;
    const fileField = action.payload.fileField;
    const {indexUpload, indexFile} = findIndexUploadAndFile(state,videoId,fileField);
    if(typeof indexUpload === "undefined"){
...
function findIndexUploadAndFile(state: State, videoId, fileField):{indexUpload?, indexFile?}{
